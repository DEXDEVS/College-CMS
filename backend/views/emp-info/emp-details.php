  <!DOCTYPE html>
  <html lang="en">
  <head>
  	<meta charset="UTF-8">
  	<title>Employee Details</title>
  </head>
  <body>

  <div class="container-fluid">
  	<section class="content-header">
        	<h1>
          	<i class="fa fa-user"></i> Employee Profile
        	</h1>
  	    <ol class="breadcrumb">
  	        <li><a href="index.php"><i class="fa fa-dashboard"></i> Home</a></li>
  	        <li><a href="index.php?r=std-personal-info">Back</a></li>
  	    </ol>
      </section>

      <!--  -->
  	<section class="content">

        <div class="row">
          <div class="col-md-3">

            <!-- Profile Image -->
            <div class="box box-primary">
              <div class="box-body box-profile">
                <img class="profile-user-img img-responsive img-circle" src="images/anas.jpg" alt="User profile picture">

                <h3 class="profile-username text-center">Anas Shafqat</h3>

                <p class="text-muted text-center"><!-- Software Engineer --></p>

                <ul class="list-group list-group-unbordered">
                  <li class="list-group-item">
                    <b>Designation</b> <a class="pull-right">Teacher</a>
                  </li>
                  <li class="list-group-item">
                    <b>Type</b> <a class="pull-right">Permanent</a>
                  </li>
                  <li class="list-group-item">
                    <b>Group</b> <a class="pull-right">Faculty</a>
                  </li>
                  <li class="list-group-item">
                    <b>Email</b> <a class="pull-right">employee@gmail.com</a>
                  </li>
                  <li class="list-group-item">
                    <b>Status</b> <a class="pull-right"><span class="label label-success">Active</span></a>
                  </li>
                </ul>

                <!-- <a href="#" class="btn btn-primary btn-block"><b>Follow</b></a> -->
              </div>
              <!-- /.box-body -->
            </div>
            <!-- /.box -->

            <!-- About Me Box -->
            
            <!-- /.box -->
          </div>
          <!-- /.col -->
          <div class="col-md-9">
            <div class="nav-tabs-custom">
              <ul class="nav nav-tabs">
                <li class="active">
                  <a href="#personal" data-toggle="tab"><i class="fa fa-user-circle"></i> Emplyee Info</a>
                </li>
                <li>
                  <a href="#refrence" data-toggle="tab"><i class="fa fa-user"></i> Employee Refrence</a>
                </li>
                <li>
                  <a href="#doc" data-toggle="tab"><i class="fa fa-book"></i> Employee Doc</a>
                </li>
              </ul>
              <!-- Employee personal info Tab start -->
              <div class="tab-content" >
                <div class="active tab-pane" id="personal">
                  <div class="row">
                    <div class="col-md-5">
                      <p style="font-size: 20px;"><i class="fa fa-info-circle" style="font-size: 20px;"></i> Personal Information</p>
                    </div>
                    <div class="col-md-2 col-md-offset-5">
                      <button class="btn btn-primary"><i class="fa fa-edit"></i> Edit</button>
                    </div>
                  </div><hr>

                  <!-- Employee info start -->

                    <div class="row">
                      <div class="col-md-6">
                        <table class="table table-striped table-hover">
                          <thead>
                            <tr>
                              <th>Employee ID:</th>
                              <td>1</td>
                            </tr>
                            <tr>
                              <th>Employee Name:</th>
                              <td>1</td>
                            </tr>
                            <tr>
                              <th>Employee Father Name:</th>
                              <td>1</td>
                            </tr>
                            <tr>
                              <th>Employee Contact No:</th>
                              <td>1</td>
                            </tr>
                            <tr>
                              <th>Employee Martial Status:</th>
                            </tr>
                            <tr>
                              <th>Employee Gender:</th>
                            </tr>
                            <tr>
                              <th>Employee Permanent Address:</th>
                            </tr>
                            <tr>
                              <td>RYK</td>
                            </tr>
                          </thead>
                        </table>
                      </div>
                      <div class="col-md-6">
                          <table class="table table-stripped">
                          <thead>
                            <tr>
                              <th>Employee Branch:</th>
                              <td>31306789214557</td>
                            </tr>
                            <tr>
                              <th>Employee CNIC:</th>
                              <td>1</td>
                            </tr>
                            <tr>
                              <th>Employee Qualification:</th>
                              <td>1</td>
                            </tr>
                            <tr>
                              <th>Employee Passing Year:</th>
                              <td>1</td>
                            </tr>
                            <tr>
                              <th>Employee Institute Name:</th>
                              <td>1</td>
                            </tr>
                            <tr>
                              <th>Employee Salary:</th>
                              <td>1</td>
                            </tr>
                            <tr>
                              <th>Employee Temporary Address:</th>
                            </tr>
                            <tr>
                              <td>RYK</td>
                            </tr>
                          </thead>
                        </table>
                      </div>
                    </div>

                  <!-- Employee info close -->

                </div>
                <!-- Employee personal info Tab close -->


                <!-- Employee Refrence tab start here -->

                <div class="tab-pane" id="refrence">
                 <div class="row">
                    <div class="col-md-5">
                      <p style="font-size: 20px;"><i class="fa fa-info-circle" style="font-size: 20px;"></i> Refrence Information</p>
                    </div>
                    <div class="col-md-2 col-md-offset-5">
                      <button class="btn btn-primary"><i class="fa fa-edit"></i> Edit</button>
                    </div>
                  </div><hr>

                  <!-- Employee refrence info start -->

                    <div class="row">
                      <div class="col-md-6">
                        <table class="table table-striped table-hover">
                          <thead>
                            <tr>
                              <th>Refrence ID:</th>
                              <td>1</td>
                            </tr>
                            <tr>
                              <th>Refrence Name:</th>
                              <td>1</td>
                            </tr>
                            <tr>
                              <th>Refrence Contact No:</th>
                              <td>1</td>
                            </tr>
                            <tr>
                              <th>Refrence CNIC:</th>
                              <td>1</td>
                            </tr>
                            <tr>
                              <th>Refrence Designation:</th>
                              <td>1</td>
                            </tr>
                          </thead>
                        </table>
                      </div>
                      <div class="col-md-6">
                         
                      </div>
                    </div>
                  <!-- Employee refrence info close -->
                </div>
                
                <!-- Employee refrence tab close here -->


                <!-- Employee Document tab start here -->

                <div class="tab-pane" id="doc">
                 <div class="row">
                    <div class="col-md-5">
                      <p style="font-size: 20px;"><i class="fa fa-info-circle" style="font-size: 20px;"></i> Document Information</p>
                    </div>
                    <div class="col-md-2 col-md-offset-5">
                      <button class="btn btn-primary"><i class="fa fa-edit"></i> Edit</button>
                    </div>
                  </div><hr>

                  <!-- Employee Document info start -->

                    <div class="row">
                      <div class="col-md-6">
                        <table class="table table-striped table-hover">
                          <thead>
                            <tr>
                              <th>Documents</th>
                              <td>If any</td>
                            </tr>
                          </thead>
                        </table>
                      </div>
                      <div class="col-md-6">
                          
                      </div>
                    </div>
                  <!-- Employee Document info close -->
                </div>
                
                <!-- Employee Document tab close here -->
            
            </div>
            <!-- /.nav-tabs-custom -->
          </div>
          <!-- /.col -->
        </div>
        <!-- /.row -->
      </section>
      <!--  -->
  </div>	
  	
  </div>	

  </body>
  </html>
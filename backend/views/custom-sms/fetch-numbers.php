<?php
	...
	$classId = $_POST['selectClass'];
 	...
 	for ($i=0; $i <$countStudents; $i++) { 
 		...
 		$contactNumbers = Yii::$app->db->createCommand("SELECT std_contact_no,std_name FROM std_personal_info WHERE std_id = '$std_id'")->queryAll();
 	...
 	echo json_encode($contactNumbers);
